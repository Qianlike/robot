...
static void timestamp_to_timeval(int64_t v, struct timeval *tv)
{
...
static void test_handler2(const lcm_recv_buf_t *rbuf, const char *channel, void *u)
{
    test_count2++;
}

static void catchall_handler(const lcm_recv_buf_t *rbuf, const char *channel, void *u)
{
    catchall_count++;
}

static void regex_handler(const lcm_recv_buf_t *rbuf, const char *channel, void *u)
{
...
#define FAIL_IFSNE(expected)                                                        \
    if (status != expected) {                                                       \
        printf("LCM: ERROR in handler registration!  %s:%d\n", __FILE__, __LINE__); \
        return 1;                                                                   \
    }
#define FAIL_IF_BAD_HID(hid)                                                        \
    if (hid == NULL) {                                                              \
        printf("LCM: ERROR in handler registration!  %s:%d\n", __FILE__, __LINE__); \
        return 1;                                                                   \
    }
#define FAIL_IF_GOOD_HID(hid)                                                       \
    if (hid != NULL) {                                                              \
        printf("LCM: ERROR in handler registration!  %s:%d\n", __FILE__, __LINE__); \
        return 1;                                                                   \
    }
