...
static PyObject *pylog_close(PyLogObject *self)
{
    if (self->eventlog) {
...
static PyObject *pylog_read_next_event(PyLogObject *self)
{
    if (!self->eventlog) {
    ...
    if (self->mode != 'r') {
        PyErr_SetString(PyExc_RuntimeError, "reading not allowed in write mode");
        return NULL;
    }

    lcm_eventlog_event_t *next_event = lcm_eventlog_read_next_event(self->eventlog);
    if (!next_event) {
        Py_INCREF(Py_None);
        return Py_None;
    }
...
static PyObject *pylog_seek(PyLogObject *self, PyObject *arg)
{
    ...
    if (self->mode != 'r') {
        PyErr_SetString(PyExc_RuntimeError, "seeking not allowed in write mode");
        return NULL;
    }
...
static PyObject *pylog_seek_to_timestamp(PyLogObject *self, PyObject *arg)
{
...
static PyObject *pylog_write_next_event(PyLogObject *self, PyObject *args)
{
    int64_t utime = 0;
    char *channel = NULL;
    ...
    if (self->mode != 'w') {
        PyErr_SetString(PyExc_RuntimeError, "writing not allowed in read mode");
        return NULL;
    }
...
static PyObject *pylog_size(PyLogObject *self)
{
    struct stat sbuf;
...
static PyObject *pylog_ftell(PyLogObject *self)
{
...
static PyObject *pylog_repr(PyLogObject *s)
{
    char buf[512];
    PyOS_snprintf(buf, sizeof(buf), "<Log object ... TODO>");
...
static int pylog_initobj(PyObject *s, PyObject *args, PyObject *kwds)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s|s", keywords, &filename, &mode))
        return -1;
