...
static void *impl_lcm_addtosubscriptiontable(lua_State *, int, int **, lcm_subscription_t ***);
static int impl_lcm_getfromsubscriptiontable(lua_State *, int, int, lcm_subscription_t **);
static int impl_lcm_removefromsubscriptiontable(lua_State *, int, int, lcm_subscription_t **);
...
static int impl_lcm_subscribe(lua_State *L)
{
    ...
    int *ref_num_ptr;
    lcm_subscription_t **subscription;
    void *userdata = impl_lcm_addtosubscriptiontable(L, 1, &ref_num_ptr, &subscription);

    /* pop subscription table */
    lua_pop(L, 1);

    /* do the actual subscribe */
    *subscription = lcm_subscribe(lcmu->lcm, channel, impl_lcm_c_handler, userdata);
 ...
 */
static void impl_lcm_c_handler(const lcm_recv_buf_t *recv_buf, const char *channel, void *userdata)
{
...
static int impl_lcm_tostring(lua_State *L)
{
    ...
    /* make the string */
    lua_pushfstring(L, "lcm.lcm [v%d.%d.%d] (@ %p)", LCM_MAJOR_VERSION, LCM_MINOR_VERSION,
                    LCM_MICRO_VERSION, lcmu);
 ...
 */
static void *impl_lcm_addtosubscriptiontable(lua_State *L, int index, int **ref_num_ptr,
                                             lcm_subscription_t ***subscription_double_ptr)
{
    ...
    *ref_num_ptr = &subu->ref_num;
    ...
    subu->ref_num = luaX_ref(L, subscription_table_index, subscription_ref_list_index);
 ...
 */
static int impl_lcm_getfromsubscriptiontable(lua_State *L, int index, int ref_num,
                                             lcm_subscription_t **subscription)
{
    /* get lcm userdata */
    impl_lcm_getsubscriptiontable(L, index);
 ...
 */
static int impl_lcm_removefromsubscriptiontable(lua_State *L, int index, int ref_num,
                                                lcm_subscription_t **subscription)
{
    /* leave nothing new on the stack */
    const int initial_top = lua_gettop(L);
